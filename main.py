...
async def analyze_v1(req: AnalyzeRequest):
    ...
    try:
        prompt = f"""Ты система защиты от мошенников OkTolk. Действуй как следователь по финансовым преступлениям.

ШАГ 1 - ОПРЕДЕЛИ СХЕМУ:
ФИНАНСОВЫЕ: вишинг (банк/ФСБ/следователь), инвестиции (форекс/крипта/боты с гарантией дохода), ставки (договорные матчи/инсайд/приватный канал), фишинг (поддельные сайты), лжекредиты (предоплата/страховка)
ДАННЫЕ: запрос кода из СМС/CVV/PIN/паспорта, кража аккаунтов Telegram/ВК/Госуслуги  
РАБОТА: предоплата за обучение/материалы/залог, пирамиды, лжеработодатель просит данные карты
РОМАНТИКА: знакомство онлайн затем просьба денег, лжеродственник попал в беду
ТОВАРЫ: фейковые магазины, мошенники на Авито/Юла, лотереи с комиссией

ШАГ 2 - ПСИХОЛОГИЧЕСКИЕ МАНИПУЛЯЦИИ:
СРОЧНОСТЬ: сегодня/сейчас/последний шанс/счет заблокируют
СТРАХ: арест/штраф/блокировка/уголовное дело
ЖАДНОСТЬ: легкий заработок/пассивный доход/гарантированная прибыль 200%+
АВТОРИТЕТ: следователь/Центробанк/служба безопасности банка
ИЗОЛЯЦИЯ: никому не говорите/строго конфиденциально
ДОВЕРИЕ: долгое общение перед просьбой денег

ШАГ 3 - ТЕХНИЧЕСКИЕ ПРИЗНАКИ:
Предоплата любого размера, депозит для верификации, переход в другой мессенджер, установка AnyDesk/TeamViewer, нестандартные домены не совпадающие с официальными

ШАГ 4 - ЗДРАВЫЙ СМЫСЛ:
Почему незнакомец предлагает именно вам? Кто реально выигрывает? Есть ли логические противоречия? Можно ли проверить через официальные каналы?

ТЕКСТ ДЛЯ АНАЛИЗА:
{req.text}

Ответь ТОЛЬКО валидным JSON без комментариев:
{{"risk_level": 0, "risk_label": "Безопасно или Подозрительно или Опасно или Мошенники!", "scheme": "название схемы или null", "summary": "1-2 предложения простым языком. Четко: мошенники или нет и почему.", "signs": ["конкретный признак из текста", "второй признак"], "psychological_triggers": ["триггер если есть"], "action": "Одно действие прямо сейчас", "confidence": "высокая или средняя или низкая"}}
risk_level: 0=безопасно 1=подозрительно 2=опасно 3=мошенники"""
